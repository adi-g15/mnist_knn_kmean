...
class Subset {
    static_assert( std::is_same_v<T, typename ContainerType::value_type>, "Container must be holding objects of same type as the subset");
	...
	public:
	Iterator(pointer ptr, Subset<T,ContainerType>& _subset, int current_index):
	...
	// Prefix increment
	Iterator& operator++() {
	    auto it = std::find(_subset._subset_repr.begin() + curr_index + 1, _subset._subset_repr.end(), true);

	    if ( it == _subset._subset_repr.end() ) {
		this->m_ptr = nullptr;
		this->curr_index = -1;
	    } else {
		// WARN: This function should be noexcept, but I still can't make sure this ->at() won't panic
		this->curr_index = std::distance(_subset._subset_repr.begin(), it);
		this->m_ptr = &(_subset._original_set[ this->curr_index ]);
	    }
	    return *this;
	}

	// Postfix increment - https://www.internalpointers.com/post/writing-custom-iterators-modern-cpp
